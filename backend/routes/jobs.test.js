...
describe("GET /coins/:id", function () {
  test("works for anon", async function () {
    const resp = await request(app).get(`/coins/${testCoinIds[0]}`);
    expect(resp.body).toEqual({
      coin: {
        id: testCoinIds[0],
        title: "J1",
  ...
  test("not found for no such coin", async function () {
    const resp = await request(app).get(`/coins/0`);
...
describe("PATCH /coins/:id", function () {
  test("works for admin", async function () {
    const resp = await request(app)
        .patch(`/coins/${testCoinIds[0]}`)
        .send({
          title: "J-New",
        })
        .set("authorization", `Bearer ${adminToken}`);
...
describe("DELETE /coins/:id", function () {
  test("works for admin", async function () {
    const resp = await request(app)
        .delete(`/coins/${testCoinIds[0]}`)
        .set("authorization", `Bearer ${adminToken}`);
    expect(resp.body).toEqual({ deleted: testCoinIds[0] });
  });
